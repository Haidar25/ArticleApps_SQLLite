...
public class ArtikelDbHelper extends SQLiteOpenHelper {
    ...
    public boolean insertData(ArtikelActivity artikelActivity){
        ContentValues val = new ContentValues();
        val.put(KEY_AUTHOR, artikelActivity.getPenulis());
        val.put(KEY_TITLE, artikelActivity.getJudul());
        val.put(KEY_ARTICLE, artikelActivity.getDeskripsi());
        val.put(KEY_CREATED_AT, artikelActivity.getCreated_at());

        long res = db.insert(TABLE_ARTICLE, null, val);
        return res != -1;
    }

    public void readData(List<ArtikelActivity> dataArtikelActivity){
        Cursor cursor = this.getReadableDatabase().rawQuery("SELECT author, title, article, created_at FROM "+TABLE_ARTICLE, null);
        while (cursor.moveToNext()){
            dataArtikelActivity.add(new ArtikelActivity(cursor.getString(0), cursor.getString(1), cursor.getString(2), cursor.getString(3)));
        }
    }
